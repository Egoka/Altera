...
### Сборщик ошибок (T-089)

- `ERROR_COLLECTOR_DRIVER` - адаптер внешнего сборщика ошибок. Поставщик не выбран (Q-01),
  поэтому доступен только `noop`, он же значение по умолчанию во всех окружениях. История
  `backend.error` пишется в таблицу `backend_error_events` при любом адаптере.
