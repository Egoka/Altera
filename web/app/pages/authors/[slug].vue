<script setup lang="ts">
  import type { ArticleCardFragment, AuthorSummaryFragment } from "~/graphql/generated/graphql"
  ...
  const mockUser: AuthorSummaryFragment = {
    id: "1",
    name: "Кот",
  ...
  const user = computed<AuthorSummaryFragment>(() => mockUser)
  ...
  const articles: ArticleCardFragment[] = [...DEMO_LATEST, ...DEMO_DEMANDED]
    ...
    .map((article, index) => ({
      ...
      author: { name: mockUser.name, slug: mockUser.handle, photoUrl: mockUser.photoUrl ?? "" }
    }))
