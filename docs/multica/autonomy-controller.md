...
Защита `app` требует актуальную ветку, поэтому любой merge в `app` делает остальные открытые PR
устаревшими. Обновлять их все после каждого merge бесполезно: следующий merge снова сделает их устаревшими.
Ветку обновляют от `app` (rebase или merge `app` в ветку) только у PR, который сливается следующим,
и непосредственно перед `merge`. Итог только что слитой задачи фиксируется в `app` до этого обновления,
чтобы не обновлять ветку дважды.
...
Порядок: `merge` → при отказе только `base` обновить ветку, дождаться CI нового head, записать новый
`tested_sha`, `pr.head_sha` и `pr.base_sha`, оставив `review` прежним, и повторить `merge`.
