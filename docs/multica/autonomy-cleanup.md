...
С 2026-09-19 PR сливаются обычным merge-коммитом (решение владельца): его второй родитель обязан совпасть
с проверенным head. Ранее слитые squash-коммиты с одним родителем тоже поддерживаются, но одной ancestry
недостаточно ни для той, ни для другой формы. Сверяется полный binary diff от подтверждённой базы PR до
проверенного head с diff от первого родителя merge-коммита до самого merge-коммита. Сравнение включает полные
blob SHA, режимы файлов, пути и удаления. Сам merge-коммит должен присутствовать в `origin/app`; live адаптер
подтверждает его серверную принадлежность PR. Неоднозначная база, пустой diff, неподдерживаемая форма merge или
расхождение дают `skipped`. Это консервативная проверка: корректное слияние с изменившейся базой тоже может
потребовать отдельного разбора вместо автоматического удаления.
...
`git worktree remove` без force. Затем заново проверяются live evidence и обе ветки. Remote-ветка не
удаляется: по решению владельца от 2026-09-19 исходная ветка PR остаётся на `origin` после слияния. Её смена
между чтениями всё равно останавливает очистку как признак новой работы. Локальная ветка удаляется
`git update-ref -d <ref> <expected-sha>`; операция сравнивает ожидаемый SHA, поэтому новая версия ветки
сохраняется. При изменении состояния после первого действия результат будет `partial`, а оставшиеся действия
остановятся.
...
Тесты создают временные Git-репозитории и локальный bare remote. Они проверяют успешные merge-коммит и squash, ошибочный diff,
