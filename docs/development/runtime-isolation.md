...
Claude 2.1.263 внутри Linux сообщает `loggedIn: false`, `authMethod: none`; macOS Keychain
не экспортировался. Его модель, context marker, фактическое отключение hooks и native
stream-json canary ещё не приняты. Native metadata diagnostic D0 не устанавливает эти факты.
...
загружаются. Source fingerprint проверяется до и после копирования и сравнивается со snapshot.
...
reasoning или default service tier через RPC также отвергается. Отключение prompt
