...
## 8. Дополнение по ходу работы

Пока шла работа, в `app` влили T-047 (`server/tests/job-queue-database.test.ts`, PR #123). Этот тест устроен
так же: 13 базовых запусков Prisma и 1 целевой, около 14,5 с на тест при таймауте 30 с. CI запускает его в том
же job, так что риск тот же. Поэтому ветка приняла свежий `origin/app` (merge), а T-047 тоже переведён на helper.
Проверки те же — AC-MT-1…AC-MT-4.
