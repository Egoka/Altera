...
| 19 | `translation.save.published` | правка любой части опубликованной статьи, включая обложку и опечатки, в копии: публичной остаётся прежняя версия, новая публикуется только после AI-проверки; при отказе AI прежняя остаётся опубликованной, автор видит причины, повторная подача — в ручную ветку ревьюера (журнал #14); на проверке одновременно одна правка статьи, её можно отозвать и продолжить; окно «перередактировать» (#100) не меняется (журнал §41 п. 2–5) | — | — | акт., свои | свои `[ДОПУЩЕНИЕ: по аналогии с #20]` | — (не правит чужой текст — журнал #12) | — | — | ✓ свои | act, own, нет другой правки на проверке | лог `translation.edit.published` (events #92) | `FORBIDDEN`, `CONFLICT`, `CONTENT_INVALID` | `30-account/author/article-edit.md` | утверждён (журнал §41); прежнее правило отменено (журнал §41 п. 1): «правка опубликованной версии только после снятия с публикации — окно «перередактировать» (#100) или снятие ревьюером (#25)» (журнал #9, #12 в этой части). Причина: автор должен иметь возможность исправить опубликованный материал — включая обложку и опечатки — без снятия его с публикации |
...
| 120 | `profile.review.decide` | решение по имени или аватару после автоматической проверки (журнал §25.4): имя — одобрить или отклонить с причиной; аватар применяется сразу, рецензент может вернуть предыдущую версию, откат аудируется (§29.5) | — | — | — | — | ✓ | — | ✓ `[ДОПУЩЕНИЕ]` | ✓ | auth, объект на проверке | `profile.check` (#85) | `NOT_FOUND`, `CONFLICT` | `30-account/reader/profile-edit.md` | утверждён (журнал §37 п. 12; новая — Г6) |
| 121 | `review.reply.author` | ответ автора рецензенту внутри конкретного решения по статье; переписка непублична (журнал §25.6) | — | — | свои решения | редакционные | читает и отвечает | — | — | ✓ | auth, own, решение открыто | `review.reply` (#86) | `VALIDATION_ERROR`, `CONFLICT` | `30-account/author/review-history.md` | утверждён (журнал §37 п. 12; новая — Г6) |
