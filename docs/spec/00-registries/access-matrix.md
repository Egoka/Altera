...
| 7 | `article.score.read` | общий результат статьи (2): автору — число без внутренних коэффициентов, формулы и фильтров (журнал §25.5); публично не показывается, объяснения позиции нет (журнал §21.18–19); AI не входит; состав по компонентам — служебным ролям | — | — | свои числа | редакционные | ✓ | ✓ | ✓ | ✓ | vis, own | — | `NOT_FOUND` (чужая статья) | `60-ranking/explainability.md` | утверждён; Г4 |
...
| 120 | `profile.review.decide` | решение по имени или аватару, направленным автоматической проверкой на дополнительную проверку (журнал §25.4): одобрить или отклонить с причиной | — | — | — | — | ✓ | — | ✓ `[ДОПУЩЕНИЕ]` | ✓ | auth, объект на проверке | `profile.check` (#85) | `NOT_FOUND`, `CONFLICT` | `30-account/reader/profile-edit.md` | черновик (новая — Г6) |
