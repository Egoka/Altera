...
- **Закрытие реестра (заход 8b, 2026-09-09)**: правила ведения — `80-observability/log-event-registry.md`;
  ...
  добавлены решениями владельца §28.2 и §28.10; строки #91–92 — решениями владельца в гейте
  блокеров 2026-09-26 (журнал §37 п. 2, §41 п. 6).
...
| 11 | `PROVIDER_UNAVAILABLE` | error | warn | api | система | `requestId`, `provider` (psp / ai / mail / storage — журнал §29.3, добавлено решением владельца при заморозке §28.3) | — | ответ клиенту + алерт | `80-observability/error-dictionary.md` | утверждён |
...
| 14 | `user.archive` / `user.restore` | audit | — | api | admin, owner (служебные записи администраторов — только owner, журнал §26.7) | `targetId`, `reasonCategory` (одна из четырёх базовых — §26.9), `publicMessage` (если добавлено), `internalReason`, `cascadeArticles`, `mode` (admin / emergency — журнал #48), `planUntil` на момент архива (срок продолжается — журнал #48, #50) | бессрочно | `/admin/audit` | `10-flows/archive-account.md` | утверждён (бывшие `user.block` / `user.unblock` — блокировка = архив, журнал #30; Г2: `mode`, план) |
...
| 53 | `system.health` | log | info | api, web | внешняя проверка | `db`, `redis`, `psp`, `ai`, `mail`, `storage` (§29.3), возраст последней успешной резервной копии базы и медиа (§29.9) | 30 дней | `/health` | `80-observability/health-and-alerts.md` | утверждён |
...
| 85 | `profile.check` | audit | — | api worker / `moderator`, `owner` | система (автоматическая проверка), сотрудник (решение) | `userId`, `field` (`name` / `avatar`), `verdict` (ok / needs_review / rejected / reverted — откат аватара к предыдущей версии, журнал §29.5), `byRole` (журнал §25.4) | бессрочно | `/admin/review`, `/admin/audit` | `30-account/reader/profile-edit.md` | утверждён (Г8b; новая — Г6) |
...
| 91 | `support.request.answered` | log | info | api | admin, owner (ответ в очереди «Обращения») | `requestId`, `topic`, статус «отвечено» — без ПДн отправителя и текста ответа в логе | 30 дней; статус обращения — бессрочно в очереди | админка «Обращения»; пользователю — статус «принято / отвечено» в кабинете (журнал §37 п. 2) | `20-public/contact.md` | принят решением владельца (журнал §37 п. 2 — добавлен решением владельца как исключение из заморозки §28.3); тип, поля и инициатор — по образцу #80 `[ДОПУЩЕНИЕ]`; код не реализован — T-119 |
| 92 | `translation.edit.published` | log | info | api | автор | `translationId`, `revisionId` (копия правки), `branch` (ai / manual) — правка опубликованной статьи в копии; публичной остаётся прежняя версия до решения AI (журнал §41 п. 2–3) | 30 дней | логи; автору — история | `30-account/author/article-edit.md` | принят решением владельца (журнал §41 п. 6 — добавлен решением владельца как исключение из заморозки §28.3) `[ДОПУЩЕНИЕ: имя кода]`; тип и поля — по образцу #45 `[ДОПУЩЕНИЕ]`; код не реализован — T-122 |
