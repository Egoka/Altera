...
|---|---|---|---|---|---|
| 1 | `40-admin/users.md` (карточка) | «Архивировать аккаунт» или «Экстренная блокировка», причина | предупреждение: число статей, оплаченный план до {дата} продолжит идти без возврата (журнал #48, #50); экстренная — дополнительное подтверждение | — | `admin.read.personal` (#67) при открытии карточки |
...
|---|---|---|---|
| 2 | «аккаунт заблокирован» с причиной по категории (§20.12) и ссылкой на оспаривание | пользователь | `/login` → форма по токену |
...
- `[ВОПРОС ВЛАДЕЛЬЦУ]` Показывать ли пользователю в письме и форме оспаривания текст причины,
  введённый сотрудником, или только категорию (§20.12)? Если текст — сотрудник пишет
  формулировку для пользователя отдельно от внутренней; если категория — как в
  `blocked-appeal.md`.
