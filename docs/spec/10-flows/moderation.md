...
|---|---|---|---|---|---|
| 1 | `40-admin/review-queue.md` | открывает очередь, «Взять в работу» старшую заявку | статья закрепляется за рецензентом, статус `in_review` — параллельные решения других недоступны (§26.5) | `reviewQueue`, `claimReview` | лог закрепления `[ДОПУЩЕНИЕ: в составе #70]` |
...
| 3а | карточка | «Запросить доработку» с рекомендациями; при желании — замечания к блокам | статус `rework` («на доработке»), закрепление снято — рецензент берёт другие материалы (§26.5); замечания привязаны к ревизии (§25.6) | `requestRework`, `createReviewNote` | `translation.rework.request` (#24), `review.message` (#70) |
...
| 5 | `article-edit.md` | автор отправляет снова | материал снова в очереди `review` без нового AI (журнал #14); любой рецензент берёт его в работу и видит историю, переписку и актуальную версию (§26.5) → шаг 1 | `submitTranslation` | `translation.submit` (#45) |
...
| Шаг 3а | автор не отвечает | статья остаётся в `rework` у автора без срока (§26.5 — срок ожидания и состояние «ждёт автора» не нужны); напоминания — проход почты (§25.13) | новая подача |
| Шаг 1 | статья закреплена за другим | `CONFLICT` «закреплена за {роль}» (§26.5) | закрепление |
...
```mermaid
flowchart TD
  A[Материал в review: отказ AI или снятие] --> B[Взять в работу: in_review, закреплена §26.5]
  B --> C{Решение}
  C -- доработка + рекомендации --> D[rework: закрепление снято; переписка, замечания к блокам]
...
- `[ДОПУЩЕНИЕ]` Код лога закрепления — в составе #70; снятие закрепления рецензентом или
  `owner`; срок ссылки #68; напоминания автору — проход почты.
...
- Закрыто Г7 (§26.5): закрепление при «взять в работу», `rework` после запроса доработки, любой
  рецензент продолжает после новой подачи; срока ожидания и статуса «ждёт автора» нет.
