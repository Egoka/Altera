...
| 4 | `admins.md` | `owner` назначает другого владельца — в два шага: сначала создана отдельная служебная запись (шаг 1), затем ей выдаётся роль `owner` (§26.7) | второй равноправный `owner` (журнал #10); назначение на произвольный e-mail недоступно | `assignOwner(id)` | `role.assign.owner` (#65) |
...
flowchart TD
  ...
  E -- назначить владельца существующей записи (2 шага §26.7) --> G[role.assign.owner]
...
- Закрыто Г7 (§26.7): владелец назначается только существующей служебной записи в два шага;
  архив и восстановление служебных записей администраторов — только `owner`.
