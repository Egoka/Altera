...
Сессия; план или выдача (`PLAN_LIMIT` иначе); рубрика, формат, теги из существующих (журнал
...
|---|---|---|---|---|---|
| 1 | `30-account/author/article-new.md` | рубрика, формат, теги, язык, заголовок | создаёт материал и версию `draft`, первую ревизию; автор — текущий аккаунт | `createArticle` | — |
...
|---|---|---|---|
| Шаг 1 | нет плана (этап платности) | `PLAN_LIMIT` → `/pricing` (журнал #5) | создание |
...
- Закрыто Г1–Г5b (журнал §6, #9, #14, #40–42, §24.3): AI — первый фильтр; повторная подача —
  ручная ветка; «перередактировать» — через AI; причины — текст проверки, комментарий ревьюера.
- `[ВОПРОС ВЛАДЕЛЬЦУ]` Уведомлять ли автора, если AI-проверка не завершилась за час (сбой
  провайдера)? Если да — письмо «задерживается» и запись в истории; если нет — только статус
  «проверяется» в кабинете.
