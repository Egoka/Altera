...
| 3 | `articles.md` | «Восстановить» (только свой архив, план активен) | ранее публичная — сразу `published` без повторной проверки (журнал §25.7); не публиковавшаяся — `draft`; медиа открыто | `restoreArticle(articleId)` | `article.restore` (#27) |
| 3о | `40-admin/articles.md` (`owner`) | «Восстановить» любую | как шаг 3: ранее публичная — сразу `published` (§25.7); аудит | `restoreArticle` | `article.restore`, `admin.change` |
...
| Рейтинг | архив / восстановление | из выдачи; при возврате в `published` судьба снимков и свежести — движок (`article-score.md`, `freshness-decay.md`) `[ДОПУЩЕНИЕ: здесь не определяется]`; рейтинг автора не штрафуется (§20.8) | — |
...
flowchart TD
  ...
  C --> D
  D --> E{Восстановление}
  E -- автор, свой архив, план активен --> F[published сразу / draft, если не публиковалась]
...
- `[ДОПУЩЕНИЕ]` Архив снимает с проверки; архивируется материал целиком; судьба снимков
  рейтинга при возврате — движок.
- Отложено (§25.13): письма; (заход 7) экран `/admin/articles`; (жалобы) архив по претензии.
- Закрыто Г1–Г2 (журнал §3–4, #53): архив обратим; иерархия акторов; `admin` архивирует без
  восстановления.
- Закрыто Г6 (§25.7): восстановление ранее публичной статьи возвращает её сразу в `published`
  без повторной проверки — и автором, и владельцем.
