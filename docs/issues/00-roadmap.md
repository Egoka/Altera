...
**Порядок этапов после первого бесплатного запуска** (журнал §42, закрывает Q-13): рейтинг и
вовлечённость (F-02) → почта: шаблоны, согласия, рассылки (F-05) → жалобы и претензии
правообладателей (F-06) → расширенное медиа (F-04) → поиск (F-03) → платность (F-01). Платность
идёт последней, что согласуется с §24.1 (после проверки ценности продукта); отдельные проходы
спецификаций будущих этапов планируются в этом порядке. Буквы стадий B–E в таблице выше — метки
содержания, а не порядок. Каталог блоков (F-07) стоит особняком: без него не завершаем
редактор стадии A; набор первого запуска решён (журнал §31), проход `95-blocks/` идёт
параллельно с T-041 (Q-02).
