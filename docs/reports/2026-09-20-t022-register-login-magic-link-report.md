...
`test` — success. Playwright — 47 passed, 9 skipped (пропущены заготовки сценариев, ожидающие
других задач). Применение миграций на изолированной базе CI —
`{"schema_version":1,"ok":true,"migrations":"passed"}`.
...
серверный редирект проверены локально на собранном Nitro. Второй прогон (run 35490283355)
нашёл четвёртый: сценарий заполнял форму до гидратации, поэтому Vue не видел введённых
значений и кнопка оставалась неактивной.
...
- **Playwright и миграция локально не запускались**: в этом runtime Docker-демон отвечает
  ...
  браузерные сценарии и применение миграции проверены прогоном CI на head ветки
  (джобы «Браузерная проверка веба» и «Сборка сервера и дымовая проверка старта»), а не
  локально. Локально на собранном Nitro без API проверена только часть сценариев.
