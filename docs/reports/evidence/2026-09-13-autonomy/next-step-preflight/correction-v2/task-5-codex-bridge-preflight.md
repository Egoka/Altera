...
The trusted run record must bind source SHA and dirty fingerprint, Multica commit, runtime/profile/run IDs, canonical native cwd, discovered task-home identity, digest of redacted non-secret policy descriptors, normalized non-secret MCP mapping digest, policy hash, image ID, model, effort, and tier. Secret values and secret-bearing raw config stay out of model-visible manifests and logs. A missing marker block, null managed config, unknown MCP transport/key, unmapped path, or mismatch between agent readback and generated block leaves `managed_mapping_verified: false`.
...
- Safe metadata-only discovery reports lexical candidate paths only; a separate trusted descriptor-relative filesystem check proves canonical per-task-home/cwd identity without reading configuration contents.
...
## Root correction history

2026-09-14: clarified digest and path evidence semantics in this actual parent-scratch preflight. The separate nested scratch/task-5-codex-bridge-preflight.md was created after a path-selection mistake; its statement that this intended file did not exist is incorrect. The original preflight already existed and is preserved in the first next-step-preflight archive. Only Milestone B prose in the native brief was changed; Milestone A requirements remain unchanged. No runtime, source, config or credential operation followed from the documentation correction.
