...
export function acceptedResult(message, id) {
  if (
    ...
    "error" in message || !message.result || typeof message.result !== "object" ||
    Array.isArray(message.result) || message.result.isError
  ) throw new Error("rpc_result_unaccepted")
  return message.result
}

function sseMessages(text) {
  return text.replaceAll("\r\n", "\n").split("\n\n").slice(0, -1).flatMap((event) => {
...
export function decodeRpc(text, contentType, id) {
  ...
  const messages = type === "application/json" ? [JSON.parse(text)] :
    type === "text/event-stream" ? sseMessages(text) : []
...
export function libraryIds(result) {
  ...
  const ids = [...text.matchAll(/^\s*-?\s*Context7-compatible library ID:\s*(\/[a-zA-Z0-9_.-]+\/[a-zA-Z0-9_./-]+)\s*$/gmi)]
    .map((match) => match[1])
...
async function trace() {
  const index = spawnSync("/usr/local/bin/trace-mcp", ["index", process.cwd()], {
    encoding: "utf8", timeout: 30000, maxBuffer: LIMIT
  })
  ...
  child.stderr.on("data", (data) => { stderr = (stderr + data.toString()).slice(0, LIMIT) })
  child.stdout.on("data", (data) => {
    bytes += data.byteLength
    if (bytes > LIMIT * 4) { fail(); child.kill("SIGTERM") }
  })
  ...
  lines.on("line", (line) => {
    ...
    } catch { /* Произвольный stdout не считается ответом MCP. */ }
  })
  async function request(method, params) {
    ...
    try {
      const response = await new Promise((resolve, reject) => {
        timer = setTimeout(() => { pending.delete(body.id); reject(new Error("trace_timeout")) }, 25000)
  ...
  try {
    ...
    if (!JSON.stringify(map).includes("sample.js") || !JSON.stringify(outline).includes("answer")) {
      throw new Error("trace_content_unaccepted")
    }
    return { command: "/usr/local/bin/trace-mcp", args: ["serve"], indexExit: 0,
      tools: ["get_project_map", "get_outline"], projectMap: true, outline: true }
  } finally {
    lines.close()
    child.kill("SIGTERM")
...
function forbidden(destination) {
  return new Promise((resolve, reject) => {
    const request = http.request({ host: "egress-proxy", port: 8080, method: "CONNECT", path: destination, timeout: 10000 })
...
export async function main() {
  ...
  for (const [name, action] of [["forbidden", async () => {
    const destinations = ["host.docker.internal:443", "127.0.0.1:443", "169.254.169.254:443",
      "mcp.context7.com:80", "mcp.context7.com.:443", "mcp.context7.com.evil.invalid:443"]
  ...
  }], ["trace", trace], ["context7", context7]]) {
    try { report.phases[name] = { ok: true, result: await action() } }
    catch (error) {
      ...
      report.phases[name] = { ok: false, error: known.test(error.message) ? error.message : "transport_or_protocol_error" }
    }
  }
