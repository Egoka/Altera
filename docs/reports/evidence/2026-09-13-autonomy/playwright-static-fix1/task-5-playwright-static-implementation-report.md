...
## Static fix1 — R1/R2 checkpoint

Base/HEAD remains 139954950e91c530772216975d1c85bce4bbd155. Fix base is the frozen stopped checkpoint; four `.before` files in `playwright-static/fix1/` preserve exact reviewed bytes. No commit/index change, Docker/browser operation, receipt, or production seccomp change. Browser consecutive failure count remains 2.

R1: actual request builders now emit saved tools/list `target` and screenshot `scale=css`. All actual screenshot paths use the same builder. Filesystem negatives require `isError`, exact attempted path, and a permission/read-only/outside-allowed-path denial while excluding schema validation errors. Relative traversal now reaches `/source` from `/runtime/evidence/playwright`. Synthetic contract mode performs no browser/container operations; its requests are validated against the three saved real input schemas and distinct denial/error controls. This is schema/classifier evidence, not an executed filesystem denial.

R2: removed raw byte substring admission. No-receipt TOML uses only the two unchanged approved mapper lexical helpers, refuses escaped/ambiguous keys, route/inline/array/nested/unknown MCP authority, and admits only exact emitted Context7 URL/rmcp and trace command/serve fields. Renaming an arbitrary executable/vector to trace is refused. Errors are fixed `playwright_policy_invalid` with suppressed exception chain. Ordinary non-MCP scalar/array values, known tables and empty emitted policy remain allowed; inline tables and multiline values are deliberately unsupported, not a general TOML parser.

Commands from execution root (all `/usr/bin/python3 -I`):

- `scripts/agent-runtime/test_playwright_mcp.py PlaywrightStaticTests.test_canary_requests_match_saved_real_schema_and_denials_are_specific`: R1 expected RED (exit1), GREEN (exit0); exact raw logs `playwright-static/fix1/r1-red.log`, `r1-green.log`.
- `scripts/agent-runtime/test_runtime.py RuntimeTests.test_no_receipt_rejects_semantic_playwright_and_ambiguous_mcp_authority`: R2 expected RED missing ValueError (exit1), GREEN (exit0), logs `r2-red.log`, `r2-green.log`.
- `scripts/agent-runtime/test_runtime.py`: 25/25 pass, `runtime-covering.log` (exit0).
- `scripts/agent-runtime/test_playwright_mcp.py`: 4/4 pass, `playwright-covering.log` (exit0).

All logs above are under exact plan scratch `playwright-static/fix1/`. Both REDs were meaningful expected regressions, not new browser attempts. Covering suites ran once after fixes; no unrelated suites/hooks. Self-review checked four-file scope, request builders at actual call sites, fixed-path denial distinctions, strict field/command checks rather than server name alone, and ordinary policy fixtures. Frozen fix-only diff: `playwright-static/fix1/fix-only.diff`, SHA256 ce716fbfc00f30fec4a914ed927c3a0780be5a6070b4be16f0d0f7d9eac13670; source/base/hash/check proof: `playwright-static/fix1/source-proof.json`. Independent scoped re-review pending.
