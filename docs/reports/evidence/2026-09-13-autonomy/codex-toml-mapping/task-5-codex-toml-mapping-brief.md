...
## Subsequent root D2 observation (mapping not yet run)

Accepted D2 commit422d5852 and binarySHA0e3f16bb910154660d5bc8c22b4e3de16f0a20504d8b79487a70c589f28eb5e3 were used in exactly one actual native diagnostic, run01a09d1e-ab73-7dfe-8fbb-50862a0326df. The allowlisted actual descriptor is task-5-d2-native-descriptor-proof.json: lexical_candidate, cwdmatchtrue, pinned source/build IDs, canonical/taskacceptance not_checked. Cleanup is independently recorded in task-5-d2-native-cleanup-proof.json; original737source paths unchanged. No real config/auth bytes were read.

Exact postrun lstat-only metadata (task-5-d2-postrun-path-metadata.json) showed UID501 workspace/task/CODEX_HOME directories mode0755, no symlink at those named components; config.toml regular0600UID501nlink1 and size<=65536. The brief therefore correctly requires no group/world write from the workspace boundary, not directory mode0700. This preliminary metadata does not replace the full held-descriptor validation, and it does not authorize reuse of this historical path for a future model invocation. First trusted mapping may inspect exactly this diagnostic artifact after implementation review; every future actual invocation must establish its own fresh descriptor.
